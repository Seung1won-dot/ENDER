...
const CODE_LINE =
  /[{};=<>[\]`$#|\\]|=>|::|->|^\s{2,}\S|^(sudo|apt|brew|npm|npx|pnpm|yarn|git|docker|curl|wget|ssh|scp|qm|pct|systemctl|pip|python|node|deno|cd|export|import|from|def|class|function|const|let|var|return|if|for|while|echo|cat|grep|sed|awk)\b|\s--?[a-z]|"[^"]*"\s*:/
