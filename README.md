...
- 복사(이미지는 클립보드에 PNG) · 다운로드 · 다른 앱으로 공유 · 고정 · 검색(`/`) · 종류 필터(텍스트·링크·이미지·파일) · 만료(1h/1d/7d/영구, 남은 기간을 눌러 연장)
- 텍스트 인라인 편집, 삭제는 5초 안에 되돌리기, 링크는 페이지 제목·파비콘 표시, 코드 조각은 모노 글꼴
- 키보드: `j`/`k` 이동, `c` 복사, `Enter` 열기·다운로드, `e` 편집, `d` 삭제, `p` 고정, `x` 만료, `/` 검색
- 오프라인이거나 조회에 실패해도 마지막 목록을 보여 주고, 탭이 숨겨진 동안 도착한 개수를 탭 제목에 표시
