...
drop policy if exists "chest owner select" on storage.objects;
...
drop policy if exists "chest owner insert" on storage.objects;
...
drop policy if exists "chest owner update" on storage.objects;
create policy "chest owner update" on storage.objects
  for update to authenticated
  using (bucket_id = 'chest' and (storage.foldername(name))[1] = auth.uid()::text)
  with check (bucket_id = 'chest' and (storage.foldername(name))[1] = auth.uid()::text);

drop policy if exists "chest owner delete" on storage.objects;
